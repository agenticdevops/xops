...
xops is a self-hosted, 24/7 personal DevOps AI agent: install via a 5-minute wizard, chat with it on Telegram (later Slack/Web), and it actually executes DevOps work — Kubernetes debugging, incident response, routine automation — safely, on your own infrastructure. It is the product/UX layer (wizard, channels, memory, docs) on top of Block's goose as the agentic execution engine.

## Core Value

A user messages xops on Telegram and it safely executes a Kubernetes operation end-to-end — with fail-closed guardrails and verification of actual cluster state, not model self-report.
...
- ✓ Config loading + Zod validation (`@xops/core`) — existing
- ✓ Setup wizard flow with Clack (`@xops/wizard`) — existing
- ✓ Telegram/Slack/Web channel adapters with chunking (`@xops/channels`) — existing
- ✓ Hybrid memory: SQLite + sqlite-vec, 400-token chunks, 70/30 vector/keyword (`@xops/memory`) — existing
- ✓ SKILL.md loader with gray-matter frontmatter parsing (`@xops/skills`) — existing
- ✓ Hono gateway server wiring channels + memory (`@xops/gateway`) — existing
...
- [ ] Build out `@xops/automation`: recipe-driven scheduled runs (heartbeat, morning briefing, cron) on the same goose+verify pipeline
...
- **Session research (2026-07-19):** Architectural comparison of three sibling projects concluded xops's value is the product shell; its engine was the weakest of the three attempts:
...
- **Codebase map:** `.planning/codebase/` (7 docs). Known concerns include: relative cross-package imports bypassing `@xops/*` workspace names, duplicate message types between core and channels, runtime-breaking interface mismatch between `apps/cli/src/index.ts` and `packages/memory/src/manager.ts`, empty `@xops/automation` package.
...
| Port openagentix skill format (SKILL.md + scripts + decision tables) | Proven with goose recipes; richer than xops's current format; lighter than aoh pack spec | — Pending |
...
| Consolidate with xopsbot: xops wins, brand xops.sh (2026-08-12) | Enterprise DevOps buyers won't trust OpenClaw; goose (Block) is credible. xops has proven harness; xopsbot becomes parts donor: risk taxonomy (186 cmds), 5 personas, wizard flows, safety modes, docs content, 10 prose skills to convert | — Pending |
